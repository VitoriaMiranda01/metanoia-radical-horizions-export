...
const PricingPeriodsManager = ({ type, periods = [], onSave }) => {
  ...
  const handleSave = async () => {
    ...
    if (!formData.start_date || !formData.end_date || !formData.value || formData.start_date.length !== 10 || formData.end_date.length !== 10) {
      setError('Todos os campos são obrigatórios e as datas devem estar completas.');
      return;
    }
    ...
    if (hasOverlap) {
      setError('O período se sobrepõe a um período já existente.');
      return;
    }
    ...
    try {
      ...
      toast({
        title: 'Período salvo',
        description: 'As configurações de período foram atualizadas no banco de dados.',
        className: 'bg-emerald-600 text-white border-none'
      });
      ...
      setError(err.message || 'Erro ao salvar o período. Tente novamente.');
      toast({
        title: 'Erro ao salvar',
        description: err.message || 'Houve um problema de conexão com o banco de dados.',
        variant: 'destructive'
      });
    } finally {
      setIsSaving(false);
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end space-x-2">
                        ...
                        <Button 
                          ...
                          disabled={isEditing || isSaving}
                        >
                          {isSaving && !isEditing ? <Loader2 className="w-4 h-4 animate-spin" /> : <Trash2 className="w-4 h-4" />}
                        </Button>
                      </div>
                    </td>
                  </motion.tr>
