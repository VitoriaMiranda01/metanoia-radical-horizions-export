...
const HomePage = () => {
  ...
  useEffect(() => {
    const fetchDates = async () => {
      try {
        ...
        const {
          data,
          error
        } = await supabase.from('configuracoes').select('data_evento_inicio, data_evento_fim').limit(1).single();
        if (error) throw error;
        if (data && data.data_evento_inicio && data.data_evento_fim) {
        ...
        console.error("Erro ao buscar datas do evento:", error);
        setFormattedDate("Data não configurada");
      } finally {
    ...
    // Subscribe to realtime updates for the configuracoes table
    const channel = supabase.channel('configuracoes_changes').on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'configuracoes'
    }, () => {
              ...
              <div className="space-y-6 text-center lg:text-left">
                ...
                duration: 0.8
                  ...
                  <h1 className="text-4xl md:text-6xl lg:text-7xl font-extrabold text-white leading-tight mb-4">
                    ...
                    <span className="text-red-500">Radical</span>{' '}
                    <span className="text-green-500">Serra</span>
                  </h1>
              ...
              delay: 0.2
                ...
                <div className="bg-black/80 backdrop-blur-xl border border-white/10 p-6 md:p-8 rounded-3xl shadow-2xl max-w-md w-full relative group">
                  ...
                  <div className="space-y-6">
                    ...
                    <Button onClick={() => navigate('/acampante')} className="w-full bg-gradient-to-r from-red-600 to-red-800 hover:from-red-500 hover:to-red-700 text-white font-bold py-6 text-lg rounded-xl shadow-lg shadow-red-900/30 transition-all transform group-hover:scale-[1.02]">Inscrever-se</Button>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
              ...
              <FadeIn className="space-y-8">
                ...
                <Button onClick={() => navigate('/equipante')} className="bg-red-600 hover:bg-red-700 text-white text-lg px-8 py-6 rounded-xl shadow-lg shadow-red-900/20 transition-all transform hover:translate-x-2">Inscrever-se</Button>
              </FadeIn>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <FadeIn>
              <div className="relative group rounded-2xl overflow-hidden shadow-2xl border border-white/10 bg-black">
                <img src="https://horizons-cdn.hostinger.com/13c6e949-152b-4918-9648-ee8b27e5e2cf/agenda-lPAze.jpeg" alt="Agenda 2026 Metanoia Radical" className="w-full h-auto object-contain transform transition-transform duration-500 group-hover:scale-[1.01]" />
              </div>
            </FadeIn>
          </div>
        </section>

        {/* --- O que é Metanoia Radical --- */}
