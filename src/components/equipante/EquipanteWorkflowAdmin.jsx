...
import { getEquipantesByWorkflowStage, updateWorkflowStage } from '@/lib/api/equipanteApi';
...
const EquipanteWorkflowAdmin = () => {
  ...
  const exportToExcel = () => {
    const ws = XLSX.utils.json_to_sheet(workflows.map(w => ({
      Nome: w.equipantes?.nome_completo || w.equipantes?.nome,
      CPF: w.equipantes?.cpf,
      'Grupo (Idade)': w.age_group === 'minor' ? 'Menor' : 'Adulto',
      'Autorização Pais': w.parental_auth_file_url ? 'Enviado' : 'Pendente',
      'Autorização Pastoral': w.pastoral_auth_status,
      Pagamento: w.payment_status,
            ...
            {workflows.map((w) => (
              <TableRow key={w.id} className="border-white/5 hover:bg-white/5">
                <TableCell className="font-medium text-white md:sticky md:left-0 md:bg-slate-900 md:z-10 md:shadow-[2px_0_5px_rgba(0,0,0,0.2)]">
                  {w.equipantes?.nome_completo || w.equipantes?.nome}
                  <div className="text-xs text-gray-500">{w.equipantes?.cpf}</div>
                </TableCell>
                <TableCell>
                  <Badge variant="outline" className={w.age_group === 'minor' ? 'border-orange-500 text-orange-400' : 'border-blue-500 text-blue-400'}>
                    {w.age_group === 'minor' ? 'Menor' : 'Adulto'}
                  </Badge>
                </TableCell>
                <TableCell>
                  {w.age_group === 'minor' ? (
                ...
                <TableCell>
                  <Select 
                    value={w.pastoral_auth_status} 
                    onValueChange={(val) => handleUpdateStatus(w.equipante_id, 'pastoral_auth_status', val)}
                  >
                ...
                <TableCell>
                  <Badge className={`workflow-badge-${w.payment_status === 'ok' ? 'ok' : 'pendente'}`}>
                    {w.payment_status}
                  </Badge>
                </TableCell>
                <TableCell>
                  <Select 
                    value={w.scale_status} 
                    onValueChange={(val) => handleUpdateStatus(w.equipante_id, 'scale_status', val)}
                  >
                    <SelectTrigger className="h-8 w-[130px] bg-transparent border-white/20 text-white text-xs">
                      <SelectValue />
                    </SelectTrigger>
