...
const getStatusBadge = (status) => {
  const variants = {
    ...
    rejeitada: 'bg-red-500/20 text-red-400 border-red-500/50',
  };
  const labels = {
    pendente: 'Pendente',
    aprovado: 'Aprovado',
...
const getColumnValue = (item, filterKey) => {
  if (filterKey === 'created_at' || filterKey === 'data_nascimento' || filterKey === 'data_inscricao') {
...
const InscricoesTable = ({ dados, tipo = 'equipantes', onSelect, onExcluir, onExportar, searchTerm, onSearchChange }) => {
  ...
  const renderCellContent = (item, key) => {
    ...
    if (key === 'created_at' || key === 'data_nascimento' || key === 'data_inscricao') return formatarData(item[key]);
        ...
        {dados.length === 0 ? (
          ...
          <>
            ...
            <div className="hidden md:block overflow-x-auto relative rounded-md border border-white/10">
              <Table>
                <TableHeader className="bg-slate-900/90 backdrop-blur supports-[backdrop-filter]:bg-slate-900/50">
                  <TableRow className="hover:bg-transparent border-white/10">
                    {/* Fixed Name Column */}
                    <TableHead className="min-w-[200px] h-12 sticky left-0 z-20 bg-slate-950 border-r border-white/10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.5)]">
                      <ColumnHeader 
                        title="Nome" 
                        filterKey="nome" 
                        filters={filters} 
                    ...
                    {visibleColumns.map(colKey => {
                      ...
                      return (
                        <TableHead key={colKey} className="min-w-[150px] h-12 bg-slate-950/50">
                            <ColumnHeader 
                    ...
                    <TableHead className="text-white text-right h-12 sticky right-0 z-20 bg-slate-950 border-l border-white/10 shadow-[-2px_0_5px_-2px_rgba(0,0,0,0.5)]">
                        Ações
                    </TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {filteredData.map((item) => (
                    <TableRow key={item.id} className="hover:bg-white/5 transition-colors border-white/10 group">
                      <TableCell className="text-white font-medium sticky left-0 z-10 bg-slate-950 group-hover:bg-slate-900 border-r border-white/10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.5)]">
                          {item.full_name || item.nome}
                      </TableCell>
                      ...
                      <TableCell className="sticky right-0 z-10 bg-slate-950 group-hover:bg-slate-900 border-l border-white/10 shadow-[-2px_0_5px_-2px_rgba(0,0,0,0.5)]">
