...
const Layout = ({ children }) => {
  ...
  const displayIdentifier = user?.name || user?.codigo || 'Usuário';
  const showEditionSelector = isOrganizer || isAprovador;
  ...
  return (
    <div className="min-h-screen bg-neutral-950 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-neutral-900 via-black to-black">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            ...
            <div className="flex items-center space-x-1 md:space-x-2">
              {showEditionSelector && (
                <div className="hidden md:flex items-center mr-4 pr-4 border-r border-white/10">
                  <EditionSelector variant="ghost" showLabel={true} />
                </div>
              )}
      ...
      {/* Mobile Edition Selector (visible only on small screens for organizers) */}
      {showEditionSelector && (
        <div className="md:hidden bg-black/60 border-b border-white/5 px-4 py-2 flex justify-center items-center">
          <EditionSelector variant="default" showLabel={true} />
        </div>
      )}
