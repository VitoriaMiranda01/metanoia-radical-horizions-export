...
import { gerarPixSicoob } from '@/lib/api/sicoobApi';
import { QRCodeSVG } from 'qrcode.react';
import { validateCPF } from '@/lib/utils';
import { useEquipanteInscriptionValue } from '@/hooks/useEquipanteInscriptionValue';
import { useCouponValidation } from '@/hooks/useCouponValidation';
import { finalizeZeroValuePayment } from '@/lib/paymentHelpers';
...
const InscricaoPixPage = () => {
  ...
  // Fetch dynamic inscription value from database
  const { value: inscriptionValue, loading: loadingValue, error: valueError } = useEquipanteInscriptionValue();
  
  const [formData, setFormData] = useState({
    nome: location.state?.nome_completo || location.state?.nome || '',
  ...
  const handleFinalizeZeroValue = async () => {
    ...
    const type = location.state?.tipo || 'acampante';
    ...
    const response = await finalizeZeroValuePayment(type, inscriptionId, appliedCoupon, userId);
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      const data = await gerarPixSicoob(valorCentavos, descricao, formData.cpf.replace(/\D/g, ''), 'acampante', formData.nome, null, appliedCoupon);
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  {isZeroValue ? (
                    <Button 
                      ...
                      disabled={isFinalizingZero || loadingValue} 
                      className="w-full h-14 text-lg font-semibold bg-emerald-600 hover:bg-emerald-700 text-white transition-all duration-300 shadow-[0_0_15px_rgba(5,150,105,0.3)] hover:shadow-[0_0_25px_rgba(5,150,105,0.5)] mt-4"
                    >
                      {isFinalizingZero ? (
                        <>
