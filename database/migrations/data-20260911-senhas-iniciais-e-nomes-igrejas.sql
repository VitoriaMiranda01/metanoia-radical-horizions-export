-- Nomes reais das igrejas + senha de primeiro acesso (custo 12).
--
-- A senha sai de _senha_primeiro_acesso(), que monta "<codigo><sufixo>" com o
-- sufixo guardado na tabela fechada config_senhas. O sufixo NAO fica aqui de
-- proposito: este repositorio e publico, e foi exatamente assim que a senha
-- antiga vazou -- escrita em texto claro num arquivo de migration.
--
-- Para preparar um ambiente novo, grave o sufixo antes de rodar este arquivo:
--   insert into config_senhas (chave, valor) values ('sufixo_primeiro_acesso', '<combinar>');
--
-- Os nomes vieram de src/constants/igrejas.js (a mesma lista dos formularios
-- de inscricao), confirmada pelo Patrick em 11/09/2026 como fonte oficial.

update public.igrejas_parceiras i
   set nome                = v.nome,
       senha               = extensions.crypt(public._senha_primeiro_acesso(i.codigo), extensions.gen_salt('bf', 12)),
